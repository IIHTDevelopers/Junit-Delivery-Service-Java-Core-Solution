...
import com.delivery.test.DeliveryTest;

public class MyApp {

	public static void main(String[] args) throws IOException {

		String sourcePath = "src/main/java/com/delivery/test/DeliveryTest.java";
		String destPath = "src/test/java/com/delivery/test/DeliveryTest.java";
		copyFile(sourcePath, destPath);
		Result result = JUnitCore.runClasses(DeliveryTest.class);
		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((linen = br.readLine()) != null) {
				...
				// Check if the third column exists and contains "DeliveryService"
				if (columns.length > 2 && columns[2].contains("DeliveryService")) {
